...
{
    unsigned char ** array = (unsigned char **)malloc(sizeof(unsigned char *)*3);
    for(int i=0; i<3; i++)
    {
        array[i] = (unsigned char *)malloc(sizeof(unsigned char)*3);
    }
    for(int i=0; i<M; i++)
    {
        for(int j=0; j<N; j++)
        {
            ...
            else
            {
                for(int i3=0; i3<3; i3++)
                {
                    ...
                    {
                        array[i3][j3] = source_data[i+i3-1][j+j3-1];
                    }
                }
                convert_data[i][j] = convert_dec(array);
            }
        }
    }
...
int main()
{
    ...
    N = 3;
    M = 3;
    unsigned char ** array = (unsigned char **)malloc(sizeof(unsigned char *)*M);
    for(int i=0; i<M; i++)
    {
        array[i] = (unsigned char *)malloc(sizeof(unsigned char)*N);
    }
    srand(time(NULL));
    for(int i=0; i<M; i++)
    {
